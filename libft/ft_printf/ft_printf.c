...
int		print(char *buff, int res, t_arg *var)
{
	...
	if ((MB_CUR_MAX == 1 && check != -1) || res == -1)
		return (-1);
	ft_bzero(g_buff, BUFF_SIZE_PRINTF);
	return (i);
}
...
int		symbol_check(char *str, t_flags *fl, t_arg *var, va_list ap)
{
	...
	while (*str)
	{
		...
		ret == -1 ? ft_bzero(&g_buff[g_k - j], BUFF_SIZE_PRINTF - g_k) : 0;
		j = MB_CUR_MAX == 1 ? j + 1 : j;
		if (*str != '\0' && g_k < BUFF_SIZE_PRINTF)
			g_buff[g_k++] = *str++;
		if (g_k >= BUFF_SIZE_PRINTF)
...
int		ft_printf(char *str, ...)
{
	...
	ft_memset(g_buff, '\0', BUFF_SIZE_PRINTF);
